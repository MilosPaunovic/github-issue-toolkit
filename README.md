...
**Epic kanban**: a List and Kanban toggle above the sub-issues list turns an epic into a board; you pick the field that forms the columns, the field that orders the cards inside each column, a value to filter the whole board down to, and a checkbox that hides the closed sub-issues. Dragging a card to another column writes the new field value, or closes and reopens the issue.
...
- An **epic kanban**: above the sub-issues list of an issue, a List and Kanban toggle draws those sub-issues as a board. The columns come from any field the sub-issues carry (or their state, type or assignee), the cards inside a column are ordered by another, a filter keeps one value of a third, and a checkbox hides the closed sub-issues; every choice is remembered. Dragging a card to another column writes the field value back to GitHub, or closes and reopens the issue. Both the board and the dragging can be switched off in the settings.
- A button that **hides the issue sidebar**: one press folds away the whole metadata column, assignees, labels, type, fields, projects and the rest, and gives the issue body and the board that width. The button sits in the gutter between the two, so it does not move when the sidebar goes, and what you chose holds on every issue. Can be switched off in the settings. A separate, optional setting hides only GitHub's **Projects** section of the sidebar and leaves the rest in place.
...
- **Hide closed** takes every closed sub-issue off the board, whatever forms the columns, so an epic in progress shows only what is left to do. The count in the toolbar says how many of the sub-issues remain, and when the columns come from the state the empty Closed column stays, so a card can still be dropped there.
- **Dragging** a card to another column moves it, when the columns come from a single-select field or from the issue state: the field value is written with `createIssueFieldValue`, the "No <field>" column clears it with `deleteIssueFieldValue`, and the state columns use `closeIssue` and `reopenIssue`, closing an issue the way it had been closed before, as completed or as not planned. The card moves first and slides back with the reason in the toolbar if GitHub refuses. Columns made of types or assignees are not drop targets. This is the only part of the board that writes, it needs a token with write access, and the whole behaviour can be switched off in the settings.
...
- Separately, **Hide the Projects section** takes GitHub's own Projects box, with its status and project fields, out of the sidebar on every issue, for a team that tracks work in issue fields and does not need it. The issue stays in its projects; only the box is hidden. Off by default.
...
- The sidebar button is a small rail `content.js` inserts between GitHub's content area and its metadata column, both flex items of the same row. Collapsing sets `display: none` on the metadata column, which is what GitHub itself does when its artifacts panel opens, and its content area grows into the space on its own. Hiding the Projects section is a class on the root element and a `content.css` rule on GitHub's `sidebar-projects-section`, so it holds however often GitHub re-renders the sidebar.
