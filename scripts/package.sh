...
#   github-issue-field-badges-<version>-chrome.zip   (Chrome, Edge, Brave and other Chromium browsers)
#   github-issue-field-badges-<version>-firefox.zip  (Firefox, loadable via about:debugging)
...
chrome_zip="$out_dir/github-issue-field-badges-${version}-chrome.zip"
rm -f "$chrome_zip"
(cd "$root" && zip -r "$chrome_zip" manifest.json "${files[@]}" >/dev/null)
echo "wrote $chrome_zip"

firefox_zip="$out_dir/github-issue-field-badges-${version}-firefox.zip"
tmp="$(mktemp -d)"
